const STORAGE_KEY = "hinomoto_pwa_state_v58";
...
function normalizeState() {
  ...
  appState.meta.appVersion = "0.58.0-response-recovery";
...
function processBridgeInboxNow() {
  let payload = null;
  ...
  // 同じ返答の二重取り込みを避ける
  if (appState.ai.lastResponse && simpleHash(appState.ai.lastResponse) === simpleHash(responseText)) {
  ...
  appState.ai.lastCheck = result;
  $("responseBox").value = responseText;
  displayChecks(result);
  ...
  localStorage.setItem(BRIDGE_INBOX_KEY, JSON.stringify({
    ...payload,
    status: "consumed",
...
function forceReceiveFromCurrentState() {
  const ok = processBridgeInboxNow();
  if (!ok) {
...
function setupEvents() {
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) setTimeout(processBridgeInboxNow, 250);
  });
  window.addEventListener("focus", () => setTimeout(processBridgeInboxNow, 250));
  setInterval(processBridgeInboxNow, 2000);
...
async function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    try { await navigator.serviceWorker.register("./service-worker.js?v=058"); }
