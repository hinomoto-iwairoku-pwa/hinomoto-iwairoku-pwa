const STORAGE_KEY = "hinomoto_pwa_state_v48";
...
function normalizeState() {
  ...
  appState.meta.appVersion = "0.48.0-candidate-actions";
...
function renderLatestCandidatePreview() {
  ...
  if (!box) return;
  const parsed = appState.ai.lastParsedResponse;
  const candidates = parsed && parsed.actionCandidates ? parsed.actionCandidates : [];
  if (!candidates.length) {
  ...
  box.innerHTML = `<div class="candidate-preview-title">採用後に反映される行動候補</div><ul>` +
    candidates.map((candidate) => `<li>${escapeHtml(stripCandidatePrefix(candidate))}</li>`).join("") +
    `</ul>`;
}
...
async function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    try { await navigator.serviceWorker.register("./service-worker.js?v=048"); }
