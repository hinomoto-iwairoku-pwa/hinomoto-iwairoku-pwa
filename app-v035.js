const STORAGE_KEY = "hinomoto_pwa_state_v35";
...
function normalizeState() {
  ...
  appState.meta.appVersion = "0.35.0-large-default-no-overlay";
...
function setupEvents() {
  ...
  if ($("closeReaderButton")) $("closeReaderButton").addEventListener("click", closeReader);
  if ($("readerAdoptButton")) $("readerAdoptButton").addEventListener("click", adoptResponse);
  if ($("readerJumpButton")) $("readerJumpButton").addEventListener("click", () => {
    closeReader();
...
async function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    try { await navigator.serviceWorker.register("./service-worker.js?v=035"); }
