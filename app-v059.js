const STORAGE_KEY = "hinomoto_pwa_state_v59";
const STORAGE_KEYS_OLD = [
  "hinomoto_pwa_state_v58",
  "hinomoto_pwa_state_v57",
...
function normalizeState() {
  ...
  appState.meta.appVersion = "0.59.0-scroll-stability";
...
function processBridgeInboxNow(options = {}) {
  ...
  // v0.59: consumed済みは無視。これがスクロール巻き戻りの主原因だった。
  if (payload && payload.status === "consumed") {
    return false;
  }

  if (!payload || !payload.responseText) {
    const possibleKeys = [
      "hinomoto_bridge_response",
      "hinomoto_bridge_last_response",
      "hinomoto_chatgpt_response"
    ];
    ...
    for (const key of possibleKeys) {
      try {
        ...
        if (maybe && maybe.status === "consumed") continue;
        if (maybe && maybe.responseText) {
          payload = maybe;
          break;
        }
        if (typeof maybe === "string") {
          payload = { responseText: maybe, requestId: appState.ai.lastRequestId || "" };
          break;
        }
      } catch (_) {
        const rawText = localStorage.getItem(key);
        if (rawText && rawText.length > 20) {
          payload = { responseText: rawText, requestId: appState.ai.lastRequestId || "" };
          break;
        }
      }
    }
  ...
  // v0.59: すでに表示中の同一返答なら何もしない。絶対にスクロールしない。
  if (appState.ai.lastResponse && simpleHash(appState.ai.lastResponse) === currentHash) {
    return false;
  }

  appState.ai.lastResponse = responseText;
  appState.ai.lastRequestId = payload.requestId || appState.ai.lastRequestId || "";
  appState.ai.lastInboxHash = currentHash;

  updateParsedResponse(responseText);
  const result = runChecks(responseText);
  appState.ai.lastCheck = result;

  if ($("responseBox")) $("responseBox").value = responseText;
  displayChecks(result);

  if (appState.ai.outbox) {
    appState.ai.outbox.status = "response_received";
    localStorage.setItem(BRIDGE_OUTBOX_KEY, JSON.stringify(appState.ai.outbox));
  }

  // v0.59: 取り込んだらinboxは消す。残すと2秒ごとに再反応する。
  localStorage.removeItem(BRIDGE_INBOX_KEY);

  saveState(result.ok ? "返答取込・検査OK" : "返答取込・検査注意");

  if (shouldFocus) {
    focusLatestResponseAfterReceive();
  } else {
    setTab("play");
    updateReadingPriorityMode();
    render();
...
function forceReceiveFromCurrentState() {
  const ok = processBridgeInboxNow({ focus: true });
  if (!ok) {
    alert("まだ取り込めるAI返答が見つかりません。ChatGPT側の生成が終わってからもう一度押してください。");
  }
}
...
function setupEvents() {
  document.addEventListener("visibilitychange", () => {
    if (!document.hidden) setTimeout(() => processBridgeInboxNow({ focus: true }), 250);
  });
  window.addEventListener("focus", () => setTimeout(() => processBridgeInboxNow({ focus: true }), 250));
  setInterval(() => processBridgeInboxNow({ focus: false }), 2000);
  document.querySelectorAll(".nav-btn").forEach((btn) => btn.addEventListener("click", () => setTab(btn.dataset.tab)));
  if ($("forceReceiveButton")) $("forceReceiveButton").addEventListener("click", forceReceiveFromCurrentState);
...
async function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    try { await navigator.serviceWorker.register("./service-worker.js?v=059"); }
