const STORAGE_KEY = "hinomoto_pwa_state_v40";
...
function normalizeState() {
  ...
  appState.meta.appVersion = "0.40.0-reader-default";
...
function updateReadingPriorityMode() {
  ...
  const shouldRead = Boolean(text && !isAdopted);
  document.body.classList.toggle("reading-priority", shouldRead);
...
function receiveExtensionResponse(payload) {
  ...
  setTab("play");
  setTimeout(updateReadingPriorityMode, 250);
}
...
function setupEvents() {
  ...
  if ($("closeReaderButton")) if ($("closeReaderButton")) $("closeReaderButton").addEventListener("click", closeReader);
  if ($("readerAdoptButton")) if ($("readerAdoptButton")) $("readerAdoptButton").addEventListener("click", adoptResponse);
  if ($("readerJumpButton")) if ($("readerJumpButton")) $("readerJumpButton").addEventListener("click", () => {
    closeReader();
...
async function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    try { await navigator.serviceWorker.register("./service-worker.js?v=040"); }
