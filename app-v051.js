const STORAGE_KEY = "hinomoto_pwa_state_v51";
...
function normalizeState() {
  ...
  appState.meta.appVersion = "0.51.0-adopt-button-fix";
...
function updateReviewButtons() {
  const text = (appState.ai.lastResponse || "").trim();
  const adoptButton = $("quickAdoptButton");
  const rejectButton = $("rejectLatestButton");
  const responseRejectButton = $("rejectButton");
  const responseAdoptButton = $("adoptButton");

  if (!text) {
    if (adoptButton) adoptButton.disabled = true;
    if (rejectButton) rejectButton.disabled = true;
    if (responseAdoptButton) responseAdoptButton.disabled = true;
    if (responseRejectButton) responseRejectButton.disabled = true;
    return;
  }

  const check = appState.ai.lastCheck || runChecks(text);
  const hasDanger = Boolean(check.dangers && check.dangers.length > 0);

  // 重大警告だけ採用不可。注意だけなら採用可能。
  if (adoptButton) adoptButton.disabled = hasDanger;
  if (responseAdoptButton) responseAdoptButton.disabled = hasDanger;
  if (rejectButton) rejectButton.disabled = false;
  if (responseRejectButton) responseRejectButton.disabled = false;
}
...
async function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    try { await navigator.serviceWorker.register("./service-worker.js?v=051"); }
