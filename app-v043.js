const STORAGE_KEY = "hinomoto_pwa_state_v43";
...
function normalizeState() {
  ...
  appState.meta.appVersion = "0.43.0-wide-document-first";
...
function renderReaderText() {
  ...
  const displayText = parsed ? (parsed.bodyText || parsed.rawText) : "";
...
function parseAiResponse(text) {
  ...
  if (!raw) {
    return { rawText: "", safetyText: "", bodyText: "", candidatesText: "", actionCandidates: [], hasBody: false, hasCandidates: false };
  }
...
function updateParsedResponse(text) {
  const parsed = parseAiResponse(text);
  appState.ai.lastParsedResponse = parsed;
  return parsed;
}
...
function renderLatestResponse() {
  ...
  if (info && parsed) {
    ...
    info.textContent = `分解結果：本文案 ${parsed.hasBody ? "あり" : "未検出"} / 行動候補 ${candidateCount}件${candidateCount ? "（採用時に候補欄へ反映）" : ""}`;
  }
...
function receiveExtensionResponse(payload) {
  ...
  setTab("play");
  setTimeout(flashLatestResponseCard, 250);
}
...
function setupEvents() {
  ...
  $("goNextActionButton").addEventListener("click", scrollToActionCard);
  if ($("closeReaderButton")) if ($("closeReaderButton")) if ($("closeReaderButton")) if ($("closeReaderButton")) if ($("closeReaderButton")) $("closeReaderButton").addEventListener("click", closeReader);
  if ($("readerAdoptButton")) if ($("readerAdoptButton")) if ($("readerAdoptButton")) if ($("readerAdoptButton")) if ($("readerAdoptButton")) $("readerAdoptButton").addEventListener("click", adoptResponse);
  if ($("readerJumpButton")) if ($("readerJumpButton")) if ($("readerJumpButton")) if ($("readerJumpButton")) if ($("readerJumpButton")) $("readerJumpButton").addEventListener("click", () => {
    closeReader();
    $("responseBox").scrollIntoView({ behavior: "smooth", block: "start" });
    $("responseBox").focus();
  });

  $("jumpToResponseButton").addEventListener("click", () => {
    $("responseBox").scrollIntoView({ behavior: "smooth", block: "start" });
    $("responseBox").focus();
  });
...
async function registerServiceWorker() {
  if ("serviceWorker" in navigator) {
    try { await navigator.serviceWorker.register("./service-worker.js?v=043"); }
